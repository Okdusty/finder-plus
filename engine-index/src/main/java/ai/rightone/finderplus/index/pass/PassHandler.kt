...
sealed interface PartialResult {
    ...
    data class LabelTags(
        val tags: List<Tag>,
        val clearSources: Set<ai.rightone.finderplus.model.TagSource> = emptySet(),
    ) : PartialResult
    data class OcrText(val text: String, val lang: String?, val keywords: List<String>) : PartialResult
    data class TranscriptSegments(val segments: List<ai.rightone.finderplus.model.Segment>) : PartialResult
    ...
    data class Faces(
        val faces: List<ai.rightone.finderplus.vision.DetectedFace>,
        val tags: List<Tag>,
    ) : PartialResult
     ...
     *
     * Carries no tags by design — see [ai.rightone.finderplus.index.pass.KeyframesPassHandler] for why
